...
// Function to write metadata file (same format as CUDA)
void write_metadata(const std::string& base_filename, const KMeansConfig& config) {
    ...
    meta_file << "K-Means OpenMP Implementation Metadata\n";
    meta_file << "======================================\n";
    meta_file << "Date: " << __DATE__ << " " << __TIME__ << "\n";
    meta_file << "Implementation: OpenMP\n";
    meta_file << "\nConfiguration:\n";
    ...
    meta_file << "  OpenMP threads: " << config.num_threads << "\n";
...
// Function to write timing results to CSV (same format as CUDA)
...
// Function to write inertia log (same format as CUDA)
...
int main(int argc, char** argv) {
    ...
    // Print configuration
    std::cout << "=== OpenMP K-Means Configuration ===" << std::endl;
    ...
    std::cout << "OpenMP threads: " << config.num_threads << std::endl;
    ...
    // Create OpenMP implementation
    KMeansOpenMP openmp_impl(config.num_threads);
    ...
    if (config.warmup_iters > 0) {
        ...
        for (int w = 0; w < config.warmup_iters; ++w) {
            ...
            openmp_impl.run(warmup_data, config, warmup_timers);
    ...
    std::vector<std::vector<double>> all_inertias;
    ...
    for (int b = 0; b < config.bench_iters; ++b) {
        ...
        // Custom run loop to track inertias
        Timer total_timer;
        total_timer.start();
        ...
        Timer init_timer;
        init_timer.start();
        openmp_impl.initialize(run_data, config);
        ...
        for (size_t iter = 0; iter < config.max_iters; ++iter) {
            ...
            Timer assign_timer;
            assign_timer.start();
            openmp_impl.assign_labels(run_data);
            ...
            Timer update_timer;
            update_timer.start();
            openmp_impl.update_centroids(run_data);
            ...
            // Compute inertia
            double inertia = openmp_impl.compute_inertia(run_data);
    ...
    // Calculate statistics (median values)
    ...
    if (!config.output_file.empty()) {
        ...
        if (config.validate) {
            ...
            openmp_impl.run(final_data, config, final_timer);
            KMeansUtils::save_results(final_data, results_file);
        }
    }

    std::cout << "\nOpenMP K-Means completed successfully!" << std::endl;
